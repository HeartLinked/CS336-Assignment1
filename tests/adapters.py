...
from cs336_basics.cross_entropy import my_run_cross_entropy
...
def run_linear(
    ...
    dtype = weights.dtype
    my_linear = Linear(d_in, d_out, device, dtype)
    # here, we define 'self.weight' in my linear, so use 'weight' as key
    my_linear.load_state_dict({"weight": weights})
...
def run_embedding(
    ...
    my_embedding = Embedding(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
...
def run_swiglu(
    ...
    swiglu = SwiGlu(d_ff=d_ff, d_model=d_model, device=w1_weight.device, dtype=w1_weight.dtype)
...
def run_rope(
    ...
    """
    my_Rope = RoPE(theta=theta, d_k=d_k, max_seq_len=max_seq_len, device=in_query_or_key.device)
...
def run_cross_entropy(
    ...
    """
    return my_run_cross_entropy(inputs, targets)
...
def get_tokenizer(
    ...
    """
    return my_get_tokenizer(vocab, merges, special_tokens)
